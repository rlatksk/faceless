...
- **`st.code` is the only element with a copy button. The upload pack uses it (`language=None, wrap_lines=True`) — a text input needs a manual select-all, which is exactly the friction issue #20 was filed about. `st.image` with no width fills the container.** The thumbnail is 1080×1920; uncapped it ate ~590px of a 391px-wide card and pushed the copy fields off screen. `width=300` holds in both the card and the Create tab.
