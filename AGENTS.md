...
Each run creates `output/YYYY-MM-DD_HHMMSS/` with `project.json`, `script.txt`, audio, images, transcript, and `final.mp4`. Auto-resume in Projects tab — checkpoints at each pipeline step.

## Architecture notes

- **LLM providers**: Ollama (local, `llama3`), DeepSeek (`deepseek-chat`), OpenRouter (`openai/gpt-4o-mini`)
- **TTS**: Edge TTS only (`synthesize_edge`, `list_edge_voices`). Default voice "Microsoft Eric".
- **Image providers**: OpenRouter via `/api/v1/images` (Gemini 3.1 Flash Lite $0.035/img, Grok Imagine $0.05/img, Gemini 3.1 Flash $0.07/img, Gemini 3 Pro $0.14/img), local SDXL/SDXL Turbo (free, slow on CPU)
...
- **diffusers local model**: Uses `float32` on CPU. First run downloads ~2GB model. SDXL Turbo uses 4 steps/guidance 0.0.
